...
export class OptimisationFacade {
  ...
  async optimiserEquitable(
    ...
    nbVehicules: number,
    maxTime: number
  ): Promise<{ results: OptimizationResult[], tranches: Adresse[][], routes: ReadonlyArray<ReadonlyArray<LatLngTuple>> }> {
    const { results, tranches } = await optimiseEquitable(
      adresses,
      nbVehicules,
      maxTime,
      (params) => this._carto.optimize({
        nbVehicules: params.nbVehicules,
        maxTimePerVehicule: params.maxTimePerVehicule,
    ...
    return { results, tranches, routes: allRoutes };
  }
  ...
  async optimiserCluster(
    ...
    if (useSimple) {
      ...
      return { results: [optimizedRoute], tranches: [[...deliveries]], routes: allDirections };
    }
    ...
    return { results: result.results, tranches: result.tranches, routes: allDirections };
  }
